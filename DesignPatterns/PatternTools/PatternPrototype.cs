...
namespace DesignPatterns
{
    ...
    public class PatternPrototype<T> : IIdableObject, IHaveIndexer where T : new()
    {
        ...
        protected void SQLAction(string sql, Func<SQLConnection, int>? getID = null)
        {
            SQLAction(sql, DomainObject, getID);
        }

        protected void SQLAction(string sql, T obj, Func<SQLConnection, int>? getID = null)
        {
            using (SQLConnection connection = new SQLConnection(SqlConnect.ConnectionString))
            {
                ...
                SQLHelper<T>.MapParameters(command, obj, Id);
        ...
        protected void InnerFetch<DomainType>(Action<T, IdWrapper<DomainType>> Do_fetch)
            where DomainType : new ()
        {
            using (SQLConnection connection = new SQLConnection(SqlConnect.ConnectionString))
            {
                connection.Open();
                ...
                using (SQLCommand command = new SQLCommand(sql, connection))
                {
                    using (SQLReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            var obj = new IdWrapper<DomainType>(new DomainType());

                            obj.Id = Convert.ToInt32(reader["Id"]);
                            SQLHelper<DomainType>.MapProperties(reader, obj._obj, -1);
