...
namespace DesignPatterns
{
    ...
    {
        public static void Run(Person[] persons, SqlConnector SqlConn)
        {
            Console.WriteLine("\nACTIVE RECORD TEST RUNNING //////////");

            var ps = new List<ActiveRecord<Person>>();
            for (int i = 0; i < persons.Length; i++)
            {
                ps.Add(new ActiveRecord<Person>(persons[i], SqlConn));
            }
            ...
            for (int i = 0; i < 10; i++)
            {
                var p = ActiveRecord<Person>.Find(i, SqlConn);
                Console.WriteLine($"{i}: " + p?.ToString() ?? "null");
            }

            var person = ActiveRecord<Person>.Find(3, SqlConn);
        ...
        public static void RunDeleteTest(SqlConnector SqlConn)
        {
            ...
            var tmp0 = ActiveRecord<Person>.Find(1, SqlConn);
            Console.WriteLine("tmp0: " + tmp0?.Id);
            Console.WriteLine("tmp0: " + tmp0?.ToString());

            var tmp = ActiveRecord<Person>.Find(1, SqlConn);
            ...
            var tmp2 = ActiveRecord<Person>.Find(1, SqlConn);
