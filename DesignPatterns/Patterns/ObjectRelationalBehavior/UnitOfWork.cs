...
namespace DesignPatterns
{
    ...
    public class UnitOfWork<T, InnerType> : ICommit
        where T : IEnumerable<IdWrapper<InnerType>>
        where InnerType : new ()
    {
        ...
        public bool Commit()
        {
            try
            {
                ...
                {
                    IdWrapper<InnerType>? tmp = null;
                    if (!command.CommandText.Contains("DELETE"))
                    {
                        tmp = _domainObject.Where(x => x.Id == id).First();
                        // FIXME
                        if (tmp == null) { return false; }
                        SQLHelper<InnerType>.MapParameters(command, tmp._obj, id);
                    }
                    else
                    {
        ...
        private (string, List<int>) InsertItems(List<IdWrapper<InnerType>> items, HashSet<int> ls)
        {
            var to_insert = items
        ...
        private (string, List<int>) UpdateItems(List<IdWrapper<InnerType>> items, HashSet<int> ls)
        {
            var to_update = items
                .Where(x => ls.Contains(x.Id))
                //.Select(x => (InnerType)Convert.ChangeType(x, typeof(InnerType)))
        ...
        private (string, List<int>) DeleteItems(List<IdWrapper<InnerType>> items, HashSet<int> ls)
        {
