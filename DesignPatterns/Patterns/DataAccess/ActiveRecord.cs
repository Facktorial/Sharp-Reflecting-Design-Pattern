...
namespace DesignPatterns
{
    public class ActiveRecord<T>
        : PatternPrototype<T>, ICRUD<T>, IFetchable<ActiveRecord<T>>, IHaveIndexer where T : new()
    {
        ...
        private static Func<SQLTypes, ActiveRecord<T>?> GetActiveRecord(SqlConnector conn, int id)
        {
            return (SQLTypes sql) => {
                ...
                SQLHelper<T>.MapProperties(sql.Reader, tmp.DomainObject, id);
                tmp.Id = id;
                return tmp;
            };
        }
        ...
        {
            var tmp = Find(Id, SqlConnect);
            if (tmp == null) { Id = -1; return this; }
        ...
        {
            return ToString(this.GetType().Name.Split('`')[0]) + $", Id: {Id}";
        }
