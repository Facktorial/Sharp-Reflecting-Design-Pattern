...
namespace DesignPatterns
{
    public class DataMapper<T>
        : PatternPrototype<List<IdWrapper<T>>>, IHaveIndexer, IFetchable<DataMapper<T>>,
        IEnumerable<IdWrapper<T>>, 
        ICRUDCollections<T> where T : new()
    {
        StateOfCollection State;
        int MaxIdx;
        private UnitOfWork<List<IdWrapper<T>>, T> _unitUploadWork;
        ...
        public DataMapper(SqlConnector sql) : base()
        {
            ...
            _unitUploadWork = new UnitOfWork<List<IdWrapper<T>>, T>(DomainObject, SqlConnect);
        }
        public DataMapper(List<IdWrapper<T>> obj, SqlConnector sql) : base(obj, -1, sql) 
        {
            Fetch();
            State = new StateOfCollection(
                new HashSet<int>(), new HashSet<int>(), new HashSet<int>()
        ...
        public DataMapper<T> Fetch()
        {
            ...
            Action<List<IdWrapper<T>>, IdWrapper<T>> fn = (ls, obj) => ls.Add(obj);
        ...
        {
            var tmp = (IdWrapper<T>) DomainObject.Select(x => x.Id == id);
            if (tmp != null)
            {
        ...
        public void Delete(Expression<Func<IdWrapper<T>, bool>> expression)
        {
        ...
        public IEnumerator<IdWrapper<T>> GetEnumerator()
        {
        ...
        public new object this[string propertyName]
        {
            get
            {
                var lsProps = typeof(List<IdWrapper<T>>).GetProperties();
